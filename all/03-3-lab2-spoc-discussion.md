...
1. X86有几个特权级？
   - CPU支持四个特权级，操作系统一般使用4个。


2. 不同特权级有什么区别？
   - 同一条指令在不同特权级下的访问权限和行为是不同的。
...
1. 一条指令在执行时会有哪些可能的特权级判断？
   - 访问数据时，要查看权限
   - 通过段寄存器ss访问数据段，需要CPL、RPL=DPL
   - 控制转移时，只能低特权级调用高特权级
   - 中断或异常时，需要判断当前的门描述符的特权级和目标代码的特权级
2. 在什么情况下会出现特权级切换？
   - 控制转移时
   - 中断或异常时
...
4. 如何利用int和iret指令完成不同特权级的切换？
   ...
   - 构造需要的栈结构，通过int和iret指令进行切换


5. TSS和Task Register的作用是什么？

   - TSS在任务切换时起重要作用，保存CPU中各寄存器的值，实现任务的挂起和恢复
